...
class signin : AppCompatActivity() {
    ...
    private lateinit var _login: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        displayName.onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                displayName.background = ContextCompat.getDrawable(applicationContext, R.drawable.login_form_edittext_focused)
            } else {
        ...
        username.onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                username.background = ContextCompat.getDrawable(applicationContext, R.drawable.login_form_edittext_focused)
            } else {
                username.background = ContextCompat.getDrawable(applicationContext, R.drawable.login_form_edittext)
            }
        }
        password.onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                password.background = ContextCompat.getDrawable(applicationContext, R.drawable.login_form_edittext_focused)
            } else {
                password.background = ContextCompat.getDrawable(applicationContext, R.drawable.login_form_edittext)
            }
        }

        signinbtn.setSafeOnClickListener {
            if (loadingLayout.visibility != View.VISIBLE) {
                if (username.text.toString().trim() != "" && displayName.text.toString()
                        .trim() != ""
                ) {
    ...
    fun sendPostRequest(userName:String, password:String, displayName: String, animator1: ObjectAnimator, animator2: ObjectAnimator, animator3: ObjectAnimator, animator4: ObjectAnimator) {
        var reqParam = URLEncoder.encode("username", "UTF-8") + "=" + URLEncoder.encode(userName, "UTF-8")
        reqParam += "&" + URLEncoder.encode("password", "UTF-8") + "=" + URLEncoder.encode(password, "UTF-8")
        reqParam += "&" + URLEncoder.encode("displayName", "UTF-8") + "=" + URLEncoder.encode(displayName, "UTF-8")
        val mURL = URL("https://www.dongkye.tech/A5/signin.php")
